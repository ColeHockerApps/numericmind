...
struct MindGamePlayView: UIViewRepresentable {

    let startPoint: URL
    let launch: MindGameLaunchStore
    let session: MindGameSessionState
    let orientation: MindGameOrientationManager
    let onReady: () -> Void
...
final class Handler: NSObject, WKNavigationDelegate, WKUIDelegate {
    ...
    private let launch: MindGameLaunchStore
    private let session: MindGameSessionState
    private let orientation: MindGameOrientationManager
    ...
    init(
        startPoint: URL,
        launch: MindGameLaunchStore,
        session: MindGameSessionState,
        orientation: MindGameOrientationManager,
    ...
    private func handleFinish(in view: WKWebView) {
        ...
        session.setActivePoint(current, base: launch.playPoint)
    ...
    private func handleFailure(in view: WKWebView) {
        ...
        session.setActivePoint(view.url, base: launch.playPoint)
        stopMarksJob()
        revealIfNeeded()
    }
